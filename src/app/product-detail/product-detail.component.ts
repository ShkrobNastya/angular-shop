  ...
  ngOnInit() {
    this.dataStorageService.fetchProduct(this.id).subscribe(
      () => {
        ...
        this.stockPresenceMessage = (this.product.stock > 10 ? 'In stock' : (this.product.stock === 0 ? 'Out of stock' : 'Almost sold out'));

        this.color = this.productDetailService.getStockColor(this.stockPresenceMessage);
      }
    );
