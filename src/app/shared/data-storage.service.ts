...
export class DataStorageService {
  constructor(private http: HttpClient, private homeService: HomeService, private productDetailService: ProductDetailService,
  ...
  fetchProductCountInCart(id: number) {
    return this.http
      .get<Cart>(`http://localhost:8000/cart/${id}`).pipe(map(order => order.count));
  }
  ...
  fetchUser(user: User) {
    return this.http
    .get<User[]>(`http://localhost:8000/users?email=${user.email}&password=${user.password}`);
  }
